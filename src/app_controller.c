...
void app_controller_thread(void)
{
    ...
    while (1)
    {
        ...
        case SM_FIRST_TIME_USE:
            ...
            if (k_msgq_get(&app_event_queue, &evt, K_MSEC(BT_DEVICE_READY_TIMEOUT_MS)) == 0)
            {
            ...
            {
                LOG_ERR("Timeout waiting for first device to be ready in SM_FIRST_TIME_USE");
                state = SM_IDLE;
                break;
            }
...
static void determine_state(void)
{
    ...
    case 2:
        ...
        state = SM_DUAL_DEVICE_OPERATION; // Two bonded devices
        break;
    default:
